...
The hosted workspace is open to visitors without sign-in. Vercel uses the public `NEXT_PUBLIC_API_URL` pointing to Render. The hosted UI exposes the local workflow: raw-event paste, mixed-format NDJSON import/export, optional WitFoo sample import, evidence, quarantine, drift, replay, approval, and rollback. Set `LOGPROOF_PUBLIC_DEMO=1` on Render to keep the demo in its separate `public-demo` data directory and stop ingestion at 1,000 stored events until reset. Single-event HTTP bodies are capped at 256 KB before buffering; batch bodies are capped at 2 MB, and search terms at 200 characters.

The hosted workspace is shared by every visitor: anyone can read, export, reset, or change all hosted records. Use only synthetic or non-sensitive sample logs. Render's filesystem may be reset on deployment, restart, or idle spin-down; the hosted workspace is not a durable evidence store. CORS limits browser origins but is not authentication. The local API also has no user authentication, so bind it to loopback or protect it with network access controls when using real data. Raw evidence and exports are readable to anyone who can reach the API. These limits reduce storage abuse, but they do not make public ingestion private or production-ready.
