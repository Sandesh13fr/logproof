...
The ten HTTP-level test cases use isolated temporary databases. They cover all ten source families; IST receipt times and UTC normalized instants; raw-byte hashes; single and batch ingestion; binary export; input-size and batch-count limits; quarantine of deeply nested input; drift; replay gates, approval and rollback; search bounds and SQL-injection-shaped input; unknown IDs; CORS; public-demo write restrictions and storage separation; and key-protected hosted ingestion, reads, export, replay, approval, and rollback. The smoke check additionally covers CSV and LEEF parsing, LEEF 1.0 tabs and a LEEF 2.0 hex delimiter, mixed-source NDJSON round trips, and the labeled parser-evaluation fixtures.

On 25 September 2026, the ten HTTP cases, smoke check, frontend lint and production build passed. `npm audit --omit=dev` and `pip-audit -r backend/requirements.txt` reported no known advisories at that time. Dependency audits are snapshots, not a guarantee against undisclosed issues.

### Public demo safety

For the hosted workspace, set `LOGPROOF_PUBLIC_DEMO=1` and a long, random `LOGPROOF_ACCESS_KEY` on Render. Vercel uses the public `NEXT_PUBLIC_API_URL` pointing to Render. The hosted site asks for the access key and holds it in browser session storage; the API requires it in the `X-LogProof-Key` header for every data and control endpoint, including event listings, evidence, exports, ingestion, simulator controls, replay, approvals, and rollback. The health endpoint stays public. Once unlocked, the hosted UI exposes the same workflow as local: raw-event paste, mixed-format NDJSON import/export, optional WitFoo sample import, evidence, quarantine, drift, replay, approval, and rollback. The key is never placed in a public frontend environment variable or URL. The access key grants shared workspace access, not separate user accounts.

If `LOGPROOF_PUBLIC_DEMO=1` is set without an access key, the previous synthetic-only safety mode applies: direct uploads, batch imports, WitFoo imports, approvals, and rollbacks are rejected, and synthetic ingestion stops at 1,000 events until reset. Single-event HTTP bodies are capped at 256 KB before buffering; batch bodies are capped at 2 MB, and search terms at 200 characters.

The hosted workspace is shared by everyone who has its access key; any of them can read, export, reset, or change all hosted records. Use only synthetic or non-sensitive sample logs. Render's filesystem may be reset on deployment or restart; the hosted workspace is not a durable evidence store. CORS limits browser origins but is not authentication. The local API also has no user authentication, so bind it to loopback or protect it with network access controls when using real data. Raw evidence and exports are readable to anyone who can reach an unrestricted local API. These controls reduce the identified exposure and storage-abuse paths; they are not a production security assessment.
